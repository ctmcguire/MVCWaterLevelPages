...
**/
function makeData(url, tsName, n) {
	...
	var suffix = "&header=true&metadata=true&md_returnfields=station_name,parametertype_name&dateformat=UNIX&from=1994-01-01";

	numData++;//Keep track of how many times makeDataByYear is called in order to properly show/hide 'loading...' text
	$.getJSON(prefix + url + suffix, function(data) {
		...
		if(chart === null)
			...
			chart.addSeries(singleSeries);//Otherwise, add the series to the already existing chart
